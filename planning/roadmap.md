...
|---|---|---|---|---|
| **M0** | Foundations | L | Stale-scaffolding cleanup; PaaS vendor pick + managed-Postgres offering; backend + frontend repo skeletons; CI; `Command` base class + dispatcher with the logic.md pipeline; history infrastructure (per-entity tables + `command_audit_log` + dispatcher-enforced capture); per-invariant isolation-primitive assignment; audit-log write timing; adapter boundary for Postgres-specific features. | — |
...
| **M8** | Cutover prep + hardening | S–M | Data import from current spreadsheets + SCA-portal exports; error-path hardening; office training; cutover plan; first real project in the tool; placeholder for post-MVP capture (stale-RFP signal etc. — tracked, not built). | M0–M7 |
...
| PaaS vendor pick + managed-Postgres offering name | ADR-0051 | **M0** |
