...
| 2026-05-21 | DRIFT-002 | `app/engine/` + `app/http/` | **Resolved (ADR-0079).** The `engine` / `http` layer-rename slot renamed `framework/` → `engine/` (the folder now states its framework-agnostic charter) and added `app/http/` as the charactered home for cross-cutting transport code; `pagination.py`, `error_handlers.py`, and `health.py` moved into it. Root cause — no charactered home for transport code + a misleadingly-named engine folder — removed. |
