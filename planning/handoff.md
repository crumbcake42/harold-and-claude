...
**Step 2.2d — Roster batch: Employee / School / Contractor / User, full-stack.** A **Case 2** step — no scoped prompt yet. The next session runs the mandatory session-head 7-signal fit check and partitions; Step 2.2d closes Step 2.2 / M1.2.

### Prompt for the next session

> Resume work. **Next is Step 2.2d — Roster batch (Employee / School / Contractor / User, full-stack).** This is a **Case 2** step: read its full brief in `planning/steps.md` § Step 2.2d, run the mandatory session-head 7-signal fit check, and partition — the brief anticipates an L-sized step with a likely seam (4-entity backend → shared frontend-abstraction design → 4-entity frontend). Cross-check `architecture.md`'s out-of-band concerns per [[check-out-of-band-concerns]]. Surface the partition options with tradeoffs; on consensus, write the revised sub-step entries to `steps.md` and fall through to Case 3 for the first sub-step.
>
> **Branch:** `m1/02-flat-roster` (the shared M1.2 branch — the Session 47 isolation fix is merged in). Per-entity checkpoint commits per [[preserve-incremental-commits]].
>
> **Key carry-forwards into 2.2d** (full list in § Open questions): every new entity born with `AuditMetadataMixin` + a `creates`-flagged create command (ADR-0072 / ADR-0075); the `User.employee_id` FK + UNIQUE lands in the Employee migration (ADR-0061); extract a shared `require_unique` helper once User's `username` is the second consumer (ADR-0071); `seed_db` coverage for all four entities; pagination designed into the shared list abstraction (School carries a few hundred rows); the shared frontend abstractions (`EntityListPage` / `DataTable` / `useEntityForm`) designed with all four entity shapes + Contract's 2.2c exemplar in view (resolves ADR-0064's "extract at the second consumer" deferral).
>
> **Process notes:** STOP-AND-CONFIRM gate applies — chat-side canvass before any code or ADR write. Frontend layout-approval gate applies — Contract's 2.2c pages are the exemplar; the four new entities are reviewed as deltas. Next ADR free: **ADR-0077**. `planning/follow-ups/session-46-followups.md` holds four queued items (a frontend container/presentational design prompt, `/eval-prompt` mid-session mode, an `/assess` skill, the routing path-shape decision) — address at the Step 2.2d head or a dedicated slot. The QA-walkthrough-guide item (Session 47, deferred) is a 2.2d carry-forward — see § Open questions.
