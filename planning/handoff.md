...
**Step 6c-iv-b — Contract-scoping retrofit. ✓ COMPLETE. ADR-0045 written.** Case 3 scoped entry — the **application half** of Step 6c-iv, applying ADR-0043's attachment model and ADR-0044's `→ project → WABundle → contract` resolution path to EmployeeRole and WA Code. Two genuine forks + a blast-radius confirmation, deliberated and approved in chat, written up as **ADR-0045**:

- **EmployeeRole `contract_id` — mandatory.** EmployeeRole gains a mandatory M:1 → Contract; schema `(id, employee_id, role_type, contract_id, rate, start_date, end_date?)`. Nullable was rejected — it resurrects the two-tier rate resolution ADR-0043 rejected, and a null-contract row's `rate` has no coherent meaning. The disjoint-ranges invariant restructures per-`(employee, role_type)` → per-`(employee, role_type, contract)` (same role at different rates on different contracts may overlap in time; within one contract, disjoint). Rate-resolution lookup keys on `(employee, role_type, contract, date)`; `change_employee_role_rate` (ADR-0039) gains a `contract` param; the ADR-0039/0041 out-of-role-range blocker predicate generalizes (no new registry entry — registry stays at 12).
- **Contract-resolution path.** A shared derivation cited by both consumers: `entity → project → WABundle → contract`. Always resolves — `create_project` guarantees a WABundle with a required `contract_id` (ADR-0044). Money values (rates, flat fees) are read-time-derived through the live `WABundle.contract_id`, never snapshotted.
- **WA Code default flat fee — read-time derivation.** Not a stored field: `Contract.code_flat_fee_schedule[wa_code.code_type]` via the contract-resolution path (WA Code stays project-scoped per ADR-0020). A code type absent from the schedule → **null/unpriced** (no blocker, no new state — disproportionate for a draft estimator). Distinct from the per-`(WA, code)` `budget`.
- **Contract-mutability consequence (noted, no new mechanism).** An `issue_wa` / `edit_wabundle` A→B contract switch re-resolves every Time Entry on the project; entries with no contract-B EmployeeRole row trip the (generalized) out-of-role-range blocker. The honest signal; absorbed by the existing default-resolution machinery. No warning-count added.
- **Blast radius confirmed bounded.** The contract dimension is a money-resolution axis only — the structural derivation graph (WA Code → Deliverable → Documents, Sample Batch → COC/Lab Report, etc.) is unaffected. **ADR-0027 not amended** — the brief anticipated a WA Code state-machine touchpoint; there is none.

Entity roster unchanged at 20. Amends ADR-0035 / 0039 / 0041 / 0020. **Step 6c-iv complete** (6c-iv-a + 6c-iv-b both landed). Cumulative tables below refreshed.
...
**Session 14b — Step 6b-residual-2 application half (blocker-resolution model, enumerative).** Step 6c-iv-b complete (ADR-0045 — EmployeeRole `contract_id`, contract-scoped rate resolution, WA Code default flat fee); **Step 6c-iv is now done** (6c-iv-a + 6c-iv-b both landed). Next is **session 14b** — apply ADR-0042's write-off / default-resolution model to the registry: the one-by-one classification pass over all 12 registry blockers (`has-default-resolution` vs. `fix-only`) plus per-blocker default-resolution command definitions. Not a new step — the enumerative half of Step 6b-residual-2, deferred behind Step 6c-iv. **Case 3 entry** — scoped prompt below. Brief in `steps.md` → Step 6b-residual-2 (session-partition block). Execution order: 14a ✓ → 6c-iv-a ✓ → 6c-iii-a ✓ → 6c-iv-b ✓ → **14b** → Step 6c-ii predicate-table ADR → 6c-iii-b → 6d.

### Prompt for the next session

> Resume work. Next is **session 14b — Step 6b-residual-2 application half**, a **Case 3 entry** (scoped). Per `_workflow.md` Case 3: read this prompt + Open questions, read `decisions.md` (**ADR-0042** write-off / default-resolution model — load-bearing, this session applies it; **ADR-0032** blocker pattern + the registry; **ADR-0028** cross-project overlap; **ADR-0033 / ADR-0039** chain-dismissal instances; ADR-0037 for the #10 RFP blocker; ADR-0044 for the #12 WA-initialization blocker), then enter planning mode — draft a session plan in chat, wait for explicit approval. STOP-AND-CONFIRM gate applies fully; do not touch files until approved.
>
> **Why this session exists:** Step 6b-residual-2 was split Option A (model / application). Session 14a (done — ADR-0042) settled the **model**: the `write_off` Note subtype + derived write-off status, the default-resolution model + coherence test, the `has-default-resolution | fix-only` binary reframe, the nuclear-option guard, `revoke_write_off`, `resolution_kind`, design pattern #14. **Session 14b is the application half** — apply that model to the blocker registry, one entry at a time.
>
> **Scope of 14b (item 1-concrete + item 3 of the Step 6b-residual-2 brief in `steps.md`):**
> 1. **12-blocker classification pass.** For each of the 12 registry blockers, decide `has-default-resolution` vs. `fix-only` using ADR-0042's coherence test (does the condition dissolve by *excluding* a well-defined entity set, vs. *creating/correcting* something?). The 12: #1 Time Entry `wa_code=null`; #2 Sample Batch `wa_code=null`; #3 Sample Batch Lab Report ∈ {missing, invalid} at closure; #4 Sample Batch COC missing at closure; #5 Time Entry no `daily_log` at closure; #6 DepFiling required-doc missing at closure; #7 open `draft`/`in_review` RFA at closure; #8 cross-project time overlap; #9 sample collection time not covered by net on-site time; #10 non-terminal RFP not `saved` at closure (candidate lone `fix-only` survivor — the saved RFP *is* what closure means); #11 Time-Entry-out-of-role-range (ADR-0039/0041; predicate keys on `(employee, role_type, contract, date)` post-ADR-0045 — already `has-default-resolution`); #12 Time Entry before head WA `initialization_date` (ADR-0044 — already classified `has-default-resolution`). #11 and #12 are pre-classified; confirm or revise them in the pass.
> 2. **Per-blocker default-resolution command definitions.** For each blocker with a default-resolution, define the command (name, signature, what it writes off, the mandatory justification). Worked example (ADR-0042): cross-project overlap (#8) → `split_entry` then write off the overlapping sliver. `split_entry` is a *pure partition* command, not itself a write-off — its full command-shape is a separate carry-forward (no home step yet); 14b defines how the #8 default-resolution *composes* split + write-off.
>
> **Output:** the registry-reclassification ADR — amends ADR-0032 (registry now carries `has-default-resolution | fix-only` per entry + the per-blocker default-resolution commands) and ADR-0028 (cross-project overlap gains its default-resolution). Chain-dismissal instances (ADR-0033 / ADR-0039) gain `write_off`-Note emission for the secondary entities. Updated cumulative tables in `handoff.md` (blocker registry).
>
> **Out of scope of 14b — do not pull in:** ADR-0042's model is settled — do not re-open the write-off model, the coherence test, or the nuclear guard. `revoke_write_off` command shape and `split_entry` command shape are carry-forwards with no home step — 14b does not define them (it only composes `split_entry` into the #8 default-resolution). The Step 6c-ii predicate-table ADR, Step 6c-iii-b, and the WA-removal model are later.
>
> **Sequencing — do not lose this:** execution order is **session 14a (done — ADR-0042) → 6c-iv-a (done — ADR-0043) → 6c-iii-a (done — ADR-0044) → 6c-iv-b (done — ADR-0045) → session 14b → Step 6c-ii predicate-table ADR → Step 6c-iii-b → Step 6d**. ADR numbers are assigned at write time — no pre-assignment. The Step 6c-ii predicate clusters are all deliberated (see the carried-forward cluster blocks in the Last session summary); that ADR is a write, gated behind 14b.
>
> **State machines locked through ADR-0045:**
...
>
> **Roster:** 20 entities through ADR-0045 (Contract #19, WABundle #20). ADR-0045 added no entity.
> **Roles (ADR-0040):** superadmin / admin / coordinator / auditor; linear hierarchy emergent from explicit `(role, command) → permitted?` table.
> **Pattern menu:** 14 design patterns cumulative through ADR-0042 (#14 — write-off / default-resolution). ADR-0043 / 0044 / 0045 added no pattern.
> **Blocker registry:** 12 entries; the ADR-0042 reframe replaces the dismissable/fix-only binary with `has-default-resolution | fix-only` — **the per-blocker reclassification pass + per-blocker default-resolution command definitions are this session (14b)**.
...
> - **Sizing watch:** 14b is a ×12 enumerative pass + command definitions. If it bloats past one window, flag a wrap point and split rather than rush — the registry ADR can land in a follow-up.
...
- Step list (current phase): `planning/steps.md` (Step 6a complete; Step 6b core complete; Step 6b-residual complete; Step 6c-i complete; Step 6c-ii deliberation complete sessions 12–13, predicate-table ADR write deferred; Step 6b-residual-2 session 14a complete — ADR-0042 write-off model — 14b application remaining; Step 6c-iii split into 6c-iii-a + 6c-iii-b; Step 6c-iii-a complete — ADR-0044; Step 6c-iv split Option A into 6c-iv-a + 6c-iv-b; **Step 6c-iv complete — 6c-iv-a (ADR-0043) + 6c-iv-b (ADR-0045 contract-scoping retrofit); session 14b (Step 6b-residual-2 application half) next**, then the Step 6c-ii predicate-table ADR, then Step 6c-iii-b (rename + WA Code reparenting + WA-removal-model remainder), then Step 6d)
