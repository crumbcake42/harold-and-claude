...
**`engine` / `http` layer rename** — a non-milestone structural slot on a new branch `m1/engine-http-rename` off `m1/02-flat-roster`. Runs before the remainder of Step 2.2d-1a; the root-cause remedy for DRIFT-002.

### Prompt for the next session

> Resume work. **`engine` / `http` layer rename** — a non-milestone structural slot. Branch `m1/engine-http-rename` off `m1/02-flat-roster`; `--no-ff` merge back at close (precedent: `m1/admin-followups`).
>
> Read the slot brief in `planning/steps.md` (§ "Inserted slot — `engine` / `http` layer rename", under Step 2.2d) and `planning/DRIFTS.md` § DRIFT-002. Run the session-head 7-signal fit check (a behaviour-preserving refactor like 2.2b-C-1 — likely one session, M).
>
> Scope: rename `app/framework/` → `app/engine/` (the framework-agnostic command engine — the name `PATTERNS.md` already uses); create `app/http/` for cross-cutting FastAPI / transport code; move `error_handlers.py` + `pagination.py` into `app/http/`; update every import across `app/` + `tests/` + `migrations/env.py` + the CLIs; rewrite `backend/app/PATTERNS.md` + `backend/CLAUDE.md`; amend ADR-0070 (layer set + names) and write the taxonomy ADR (**ADR-0079**); set DRIFT-002's catalog status to resolved; update planning-doc references to `framework/`. Behaviour-preserving — no logic change, no migration, no OpenAPI surface change.
>
> Done when: backend runs on `engine/` + `http/`; tests + ruff + pyright green; docs + ADR-0070 updated; ADR-0079 written; DRIFT-002 resolved; `--no-ff` merge back to `m1/02-flat-roster`.
>
> **After the slot:** the remainder of **Step 2.2d-1a** (Employee + Contractor slices, the Alembic migration incl. `User.employee_id` FK/UNIQUE, `seed_db` coverage, OpenAPI regen) resumes on `m1/02-flat-roster`, built on the post-rename structure. See the Step 2.2d-1a brief in `steps.md` — its **Partially complete** note records what landed (pagination substrate / ADR-0078, the `require_unique` extraction) and the settled entity attributes. Carry-forwards for that work: Employee + Contractor each born with `AuditMetadataMixin` + a `creates`-flagged create command (ADR-0072 / ADR-0075); the migration adds the `User.employee_id` FK + UNIQUE (ADR-0061); per-entity checkpoint commits; migration applied to Neon per [[project-neon-current-policy]].

## Pointers

- Workflow protocol: `planning/_workflow.md`
- File rules registry (generated): `planning/_file-rules.md` (last regenerated 2026-05-20)
- **Drift registry:** `planning/DRIFTS.md` — catalog of drift kinds + instance log; all surfaced drift is logged here. Currently 2 kinds: `DRIFT-001` (parallel-definition drift), `DRIFT-002` (layer-charter erosion — `pagination.py` in `framework/`; the `engine`/`http` rename slot resolves it).
...
- Decisions log: `planning/decisions.md` (currently ADR-0001 through ADR-0078; next ADR at write time: **ADR-0079**)
