...
**Intent (not up for re-deliberation):** the backend structure is and stays hexagonal. 2.2b-A corrects M1.1/M1.2's drift so the layout is *accurately* hexagonal and writes it down as enforced conventions (`PATTERNS.md`). A vertical/Django rewind and a "hybrid" are both off the table — per-entity folders inside `domain/` are plain hexagonal (the domain layer organized by entity), not a deviation. The forks below are line-drawing *within* hexagonal, not whether to be hexagonal.

- **`framework/` line-drawing.** `framework/` is currently an incohesive grab-bag — the command engine (`command`/`dispatcher`/`caller`/`crud`/`exceptions`/`runtime`), concrete I/O (`db`/`adapter`/`capture`/`history`/`locks` + auth hashing/sessions), and transport (`error_handlers`). The hexagonal boundary **will** be established — that is the drift correction, not optional. The fork is where the lines fall: what counts as the application-core/engine, what is an adapter (→ the empty `adapters/`), where transport sits (`error_handlers` → a `routes/`-adjacent home). Open sub-question: is the dispatcher / `Command` base its own application-core layer, or part of `domain/`? (`db` as a folder name is rejected outright — it names a fraction of the contents.)
...
>
> **Session 41 is Step 2.2b-A — M1.2 closeout + backend conventions.** Open with a STOP-AND-CONFIRM canvass of the four structure forks in `handoff.md` § Open questions "For Step 2.2b-A": (1) `framework/` line-drawing — the hexagonal engine/`adapters`/transport boundary is established (drift correction, not optional); settle where the lines fall; (2) `routes/` shape — per-resource folders vs. flat + schemas module; (3) DTO vs command `Payload` — keep separate vs. collapse (agent rec ~70% keep separate); (4) `entities.py` vs `models.py` + audit-column scope. **The plural-naming rule and the per-entity-domain-folder layout are already settled — do not re-deliberate them.**
>
