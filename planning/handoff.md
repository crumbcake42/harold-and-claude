...
**Step 2.2d-1 — Roster backend: Employee / School / Contractor / User-admin-CRUD.** A Case 3 scoped session — the first sub-step of Step 2.2d (roster batch), on the shared branch `m1/02-flat-roster` (the `m1/admin-followups` slot has merged back with `--no-ff`).

### Prompt for the next session

> Resume work. **Step 2.2d-1 — Roster backend: Employee / School / Contractor / User-admin-CRUD.** A Case 3 scoped session. Continue on `m1/02-flat-roster` — the `m1/admin-followups` slot has merged back; **do not branch**.
>
> **Run the 7-signal fit check at the session head first.** Step 2.2d-1 is **yellow on signal 3** (four entity backends; estimate M–L) — if the check fires, propose a 2+2 split before scoping the work.
>
> Read the Step 2.2d-1 brief in `planning/steps.md` plus the Step 2.2d umbrella (backend in-scope items 1–6). Inputs: 2.2a's settled `require_role` factory + `crud.py` helpers + `seed_db` framework; `app/features/contracts/` (the backend slice reference) and `app/auth/` (the User model); `backend/app/PATTERNS.md`; `data-model.md` § Employee / School / Contractor / User; ADR-0047 Cluster 1; ADR-0061 § `user.employee_id` carry-forward; ADR-0070–0075.
>
> **Head decision (STOP-AND-CONFIRM gate):** the pagination **contract** — read-route query-param + response-envelope shape, uniform across the four entities (School carries a few hundred rows; the pagination *UI* is 2.2d-2, but the *contract* is settled here so the backend emits it). Possibly ADR-worthy — **ADR-0078** is free. Sub-decision: whether to retrofit Contract's `GET /contracts` onto the same envelope.
>
> Carry-forwards: each new entity born with `AuditMetadataMixin` + a `creates`-flagged create command (ADR-0072 / ADR-0075); extract the shared `require_unique` helper from Contract's `_require_unique_number` once User's `username` is the second consumer (ADR-0071); `seed_db` coverage for all four entities; the migration includes the `User.employee_id` FK + UNIQUE alter (ADR-0061). Per-entity checkpoint commits; migration applied to Neon at landing per [[project-neon-current-policy]]; OpenAPI contract + client regenerated.
