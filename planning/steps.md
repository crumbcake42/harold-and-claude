...
**✓ COMPLETE (2026-05-16, ADR-0052 + `planning/architecture.md`).** Case-3 scoped — the data-layer pair + the one-page sketch. Two sub-decisions bundled into ADR-0052 (engine + history-impl coupling parallels ADR-0051's three-way bundle). **(D1) Persistence engine → PostgreSQL 15+** via SQLAlchemy 2.0 + Alembic. Canvassed against MySQL/MariaDB (weaker `SERIALIZABLE` + connection-scoped `GET_LOCK` is wrong primitive), SQLite-everywhere with Litestream (categorically violates ADR-0010 isolation), and distributed-Postgres flavors (overkill at MVP scale). Postgres falls out of the ADR-0051 envelope as effectively the only honest answer. **(D2) History implementation shape → per-entity append-only history tables** (9 tables: 3 comprehensive — Document/WA/RFA; 6 lifecycle — Project/Sample Batch/Deliverable/EmployeeRole/WA Code/ContractorEngagement) + **shared `command_audit_log`** with polymorphic `(entity_type, entity_id)` for the 7 audit-log entities + nothing for the 5 no-history entities. Capture enforced in the `Command` base class + dispatcher (carry-forward from ADR-0051); ADR-0008 atomicity guaranteed by same-txn write. Rejected: single polymorphic history table (orphan-row risk + shared-column nullables), temporal tables (wrong dependency direction — capture at row-mutation surface inverts ADR-0008's command-boundary enforcement), event store (already foreclosed by ADR-0007 + ADR-0008). 10 sub-commitments pinned in ADR-0052 (common metadata, comprehensive/lifecycle column shapes, audit-log table shape, no-history entities, reference-snapshotting restatement of ADR-0013, schema evolution, capture enforcement, cross-entity invariant isolation primitives, engine-portability discipline). **ADR-0010's deferred isolation-mechanism question answered here:** `SERIALIZABLE` default + `pg_try_advisory_xact_lock` opt-in. `planning/architecture.md` drafted (component diagram + boundary semantics + 10-step successful-command data flow + out-of-band concerns + pointers). `_file-rules.md` regenerated (architecture.md added; last-regen 2026-05-16).
